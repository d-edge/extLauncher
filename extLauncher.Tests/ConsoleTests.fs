...
let Terminal (strReader: Queue<string>) (keyReader: Queue<Console.TerminalKey>) (buffer: ResizeArray<string>) =
    let mutable left = 0
    let mutable top = 0
    ...
    { new Console.ITerminal with
        ...
        member _.GetCursorPosition() = left, top

        member _.SetCursorPosition(l, t) =
            left <- l
            top <- t

        member _.ReadKey() = keyReader.Dequeue()
        member _.ReadLine() = strReader.Dequeue()
...
let backspaceKey =
    ConsoleKey.Backspace, char ConsoleKey.Backspace, noConsoleModifier

let escapeKey = ConsoleKey.Escape, char ConsoleKey.Escape, ConsoleModifiers.Alt

let aKey k =
    enum<ConsoleKey> (Char.ToUpper k |> int), k, noConsoleModifier
...
let ``prompt should clear when exit`` () =
    ...
    let keyReader =
        Queue [ ConsoleKey.Escape, char ConsoleKey.Escape, noConsoleModifier ]
